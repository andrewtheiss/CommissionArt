...
def test_multiple_generic_hubs_for_same_owner(setup):
    ...
    assert hub1_address in profile_hubs
    assert hub2_address in profile_hubs
...
def test_link_hubs_to_profile(setup):
    ...
    # Temporarily disconnect ProfileHub from OwnerRegistry
    owner_registry.setProfileHub("0x0000000000000000000000000000000000000000", sender=deployer)
    ...
    # Reconnect ProfileHub to OwnerRegistry
    owner_registry.setProfileHub(profile_hub.address, sender=deployer)
    profile_hub.setOwnerRegistry(owner_registry.address, sender=deployer)
...
def test_ensure_profile_creation(setup):
    ...
    assert profile_hub.hasProfile(user2.address) is False
...
def test_automatic_profile_creation_with_events(setup):
    ...
    assert profile_hub.hasProfile(user3.address) is False
    
    # Create a profile for the user first
    profile_hub.createProfile(sender=user3)
    
    # Verify the profile was created
    assert profile_hub.hasProfile(user3.address) is True
    profile_address = profile_hub.getProfile(user3.address)
...
def test_generic_hub_with_existing_profile(setup):
    ...
    assert profile_hub.getProfile(user1.address) == user1_profile 
...
def test_full_profile_verification(setup):
    ...
    assert profile_hub.hasProfile(new_user.address) is False
    assert owner_registry.getCommissionHubCountForOwner(new_user.address) == 0
    
    # STEP 2: Create a profile for the user first
    profile_hub.createProfile(sender=new_user)
    
    # Verify the profile was created
    assert profile_hub.hasProfile(new_user.address) is True
    profile_address = profile_hub.getProfile(new_user.address)
    ...
    # STEP 5: Verify in ProfileHub
    user_profile_from_hub = profile_hub.getProfile(new_user.address)
    assert user_profile_from_hub == profile_address, "Profile address mismatch in ProfileHub"
    
    # Verify user is in the latest users list
    latest_user_index = profile_hub.userProfileCount() - 1
    if latest_user_index < 1000:  # Only check if within bounds of latestUsers array
        latest_user = profile_hub.getLatestUserAtIndex(latest_user_index)
    ...
    assert profile.hub() == profile_hub.address, "Profile hub mismatch"
...
def test_automatic_profile_creation(setup):
    ...
    # Make sure the OwnerRegistry is properly connected to ProfileHub
