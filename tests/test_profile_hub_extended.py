...
def test_create_profile_already_exists(setup):
    ...
    first_profile = profile_factory_and_regsitry.getProfile(user.address)
    
    # Create profile again - should return the same profile, not fail
    profile_factory_and_regsitry.createProfile(sender=user)
    second_profile = profile_factory_and_regsitry.getProfile(user.address)
...
def test_get_user_profiles_pagination(setup):
    ...
    for i in range(num_users):
        ...
        user_profile = profile_factory_and_regsitry.getProfile(test_users[i].address)
    ...
    latest_users = profile_factory_and_regsitry.getLatestUserProfiles()
    ...
    for i in range(num_users):
        ...
        assert profile_factory_and_regsitry.hasProfile(test_users[i].address), f"User {i} should be registered as having a profile"
        print(f"Verified user {i}: {test_users[i].address} -> Profile: {user_profile}")
    
    # Test getAllUsersByOffset for pagination
    # Get first page
    first_page = profile_factory_and_regsitry.getAllUsersByOffset(0, 3, False)
    ...
    if user_count > 3:
        second_page = profile_factory_and_regsitry.getAllUsersByOffset(3, 3, False)
    ...
    reverse_page = profile_factory_and_regsitry.getAllUsersByOffset(0, 3, True)
    ...
    empty_page = profile_factory_and_regsitry.getAllUsersByOffset(1000, 3, False)
    assert len(empty_page) == 0, "Should return empty list for offset beyond data"
    
    # Zero count
    zero_page = profile_factory_and_regsitry.getAllUsersByOffset(0, 0, False)
