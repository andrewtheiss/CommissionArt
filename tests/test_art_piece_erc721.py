...
def setup():
    ...
    # Deploy template contracts needed for ProfileFactoryAndRegistry
    ...
    # Deploy an ArtPiece template for code hash whitelist
    ...
    # Deploy ArtCommissionHub contract
    commission_hub = project.ArtCommissionHub.deploy(sender=deployer)
    
    # Initialize the ArtCommissionHub contract
    ...
    # generic_token_id = int.from_bytes(deployer.address, byteorder='big')
    
    # Register a generic commission hub with deployer as owner
...
#     # Verify detachment
#     assert art_piece.getArtCommissionHubAddress() == ZERO_ADDRESS, "Art piece should not be attached to any hub"
